...
// UART1: 雷达数据输出到C3显示面板 (GPIO18=TX, GPIO19=RX)
...
bool sim_mode = false;
bool wifi_running = true;     // WiFi状态 (被 loop 状态机和串口命令共享)
...
void loop() {
    ...
    // 2) 读取雷达数据 (单雷达)
    radar.readFrame();
    radar.checkStale(500);
    
    // 2.1) 发送雷达数据到 UART1 (C3显示面板)
    sendRadarUART1();
    ...
    if (Serial.available()) {
        ...
        if (line == "BEEP") {
            ...
            sim_mode = !sim_mode;
            Serial.print("[FWD] SIM mode: ");
            Serial.println(sim_mode ? "ON" : "OFF");
        }
        else if (line == "WIFI") {
            Serial.println("[FWD] WiFi ON (force)");
            WiFi.mode(WIFI_AP);
    ...
    // 2.7) C3串口配置命令接收 (C3→ESP32)
    if (Serial1.available()) {
        String line = Serial1.readStringUntil('\n');
        line.trim();
        if (line.startsWith("$CFG,")) {
            int v[11];
            int n = sscanf(line.c_str(), "$CFG,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d",
                &v[0],&v[1],&v[2],&v[3],&v[4],&v[5],&v[6],&v[7],&v[8],&v[9],&v[10]);
            if (n >= 9) {
                config.rcw.low_speed       = v[0];
                config.rcw.speed_threshold = v[1];
                config.rcw.range_limit     = v[2];
                config.rcw.hold_time       = v[3];
                config.rcw.lflash_interval = v[4];
                config.rcw.flash_interval  = v[5];
                config.turn.speed_threshold= v[6];
                config.turn.range_limit    = v[7];
                config.sys.bsd_beep_cooldown = v[8];
                if (n >= 11) {
                    config.radar.det_range  = v[9];
                    config.radar.sensitivity= v[10];
                }
                config.saveToNVS();
                radar.setBSDMode();
                Serial.println("[C3] Config saved from touch panel");
            } else {
                Serial.print("[C3] Parse err: got "); Serial.print(n); Serial.println(" values");
            }
        } else if (line == "$GETCFG") {
            // 响应C3的配置请求
            static StaticJsonDocument<4096> cfg_doc;
            cfg_doc.clear();
            config.toJson(cfg_doc);
            char buf[200];
            ...
            Serial.println("[C3] Config sent");
        } else if (line.startsWith("$WIFI,")) {
            int on;
            if (sscanf(line.c_str(), "$WIFI,%d", &on) == 1) {
                if (on) {
                    Serial.println("[C3] WiFi ON");
                    WiFi.mode(WIFI_AP);
                    WiFi.softAP(config.sys.wifi_ssid, config.sys.wifi_pass);
                    server.begin();
                } else {
                    Serial.println("[C3] WiFi OFF");
                    server.end();
                    delay(50);
                    WiFi.softAPdisconnect(true);
                    WiFi.enableAP(false);
...
// ===============================================================
//  雷达数据发送到 UART1 (C3显示面板) + 模拟模式
// ===============================================================
void sendRadarUART1() {
    static unsigned long last_send = 0;
    if (millis() - last_send < 200) return;
    last_send = millis();
    ...
    if (millis() - last_hb > 1000) {
        ...
        uint8_t hb[] = {
            0xAA, 0xFF,
            (uint8_t)(bsd_l_active|bsd_r_active ? 1:0),
            (uint8_t)(rcw_l_active|rcw_r_active ? 1:0),
            (uint8_t)buzzer_mode, (uint8_t)turn_state, 0x55
        };
        Serial1.write(hb, sizeof(hb));
    }
    ...
    if (!f || !f->valid) {
        ...
        if (sim_mode) {
            ...
            if (millis() - sim_t > 500) {
                sim_t = millis();
                int8_t ang1 = -15 - (sim_t/2000)%30;
                int8_t ang2 =  10 + (sim_t/1500)%25;
                uint8_t rng1 = 5 + (sim_t/3000)%35;
                uint8_t rng2 = 8 + (sim_t/2500)%25;
                int8_t vel1 = 2 + (sim_t/4000)%4;    // 2~6 m/s (接近, 正数)
                int8_t vel2 = 1 + (sim_t/3500)%4;    // 1~5 m/s (远离, 正数)
                uint8_t sim[] = {0xAA,0x02,(uint8_t)ang1,rng1,(uint8_t)vel1,(uint8_t)ang2,rng2,(uint8_t)vel2,0x55};
                Serial1.write(sim,sizeof(sim));
            }
        }
        return;
    }
    ...
    buf[idx++]=0xAA; buf[idx++]=(uint8_t)f->obj_num;
    for(int i=0;i<f->obj_num&&i<4;i++){
        buf[idx++]=(uint8_t)(int8_t)f->objects[i].angle;
        buf[idx++]=(uint8_t)f->objects[i].range;
        buf[idx++]=(uint8_t)(int8_t)f->objects[i].velocity;
    }
    buf[idx++]=0x55;
